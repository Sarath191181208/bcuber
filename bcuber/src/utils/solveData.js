...
export class SolveData {
   ...
   */
  getCheckpointSegmentsMoves() {
    const segments = [];
    const sortedMoves = this.getSortedMoves().map((m) => ({
      move: m.move,
      timestamp: (m.timestamp - this.startTime )/ 1000,
    }));
    let start = 0;
    for (let i = 0; i < this.checkpoints.length; i++) {
      const end = this.checkpoints[i];
      const moves = sortedMoves.filter(
        (m) => m.timestamp >= start && m.timestamp < end
      );
      segments.push(moves.map((m) => m.move).join(" "));
      start = end;
    }
    return segments;
  }
  ...
  /**
   * @returns {Array<{move: string, timestamp: number}>}
   */
  getSortedMoves() {
    return this.moves.sort((a, b) => a.timestamp - b.timestamp);
  }
